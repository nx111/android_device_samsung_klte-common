...
int radio::getDataRegistrationStateResponse(int slotId,
...
#endif
    int dataRegRespStrings =
         property_get_int32("ro.ril.telephony.data_reg_resp_strings", 6);
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL) {
            ...
            int numStrings = responseLen / sizeof(char *);
            if ((numStrings != dataRegRespStrings) && (numStrings != 11)) {
                RLOGE("getDataRegistrationStateResponse Invalid response: (numStrings(%d) != %d/11)", numStrings, dataRegRespStrings);
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
                char **resp = (char **) response;
...
int radio::getOperatorResponse(int slotId,
...
#endif
    int operRespStrings = property_get_int32("ro.ril.telephony.oper_resp_strings", 3);
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        hidl_string numeric;
        int numStrings = responseLen / sizeof(char *);
        if (response == NULL || numStrings != operRespStrings) {
            RLOGE("getOperatorResponse Invalid response: numStrings(%d) != ro.ril.telephony.oper_resp_strings(%d))",numStrings,operRespStrings);
            if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else {
            char **resp = (char **) response;
...
int radio::getNetworkSelectionModeResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        bool manual = false;
        if (response == NULL || responseLen != sizeof(int)) {
            RLOGE("getNetworkSelectionModeResponse Invalid response: NULL (responseLen:%d != sizeof(int):%d)",responseLen, sizeof(int));
            if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else {
            int *pInt = (int *) response;
